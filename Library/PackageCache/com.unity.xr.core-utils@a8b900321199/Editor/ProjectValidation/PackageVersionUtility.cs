...
namespace Unity.XR.CoreUtils.Editor
{
    ...
    public static class PackageVersionUtility
    {
        ...
        static void UpdatePackageVersionsAsync()
        {
            if (s_PackageCache == null)
                s_PackageCache = new Dictionary<string, PackageVersion>();

            if (s_ListRequest == null || s_ListRequest.Status != UnityEditor.PackageManager.StatusCode.InProgress)
            {
                s_ListRequest = UnityEditor.PackageManager.Client.List(true, true);
                EditorApplication.update += CheckListRequestProgress;
            }
        }

        static void CheckListRequestProgress()
        {
            if (s_ListRequest.IsCompleted)
            {
                ...
                {
                    Debug.LogWarning($"Failed to get package versions: {(s_ListRequest.Error != null ? s_ListRequest.Error.message : "no result returned.")}");
                }
