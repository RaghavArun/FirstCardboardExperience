...
namespace Unity.XR.CoreUtils
{
    ...
    public static class NativeArrayUtils
    {
        ...
        /// <summary>
        /// Ensure that this array is the exact size of the given capacity.
        /// </summary>
        /// <remarks>
        /// If the array is not the necessary size, it is disposed and a new, empty array is created.
        /// </remarks>
        /// <typeparam name="T">The type of array element.</typeparam>
        /// <param name="array">The array reference. Overwritten if the original array has different size.</param>
        /// <param name="size">The exact number of elements that the array must be able to contain.</param>
        /// <param name="allocator">The allocator to use when creating a new array, if needed.</param>
        /// <param name="options">The options to use when creating the new array, if needed.</param>
        public static void EnsureExactSize<T>(ref NativeArray<T> array, int size, Allocator allocator, NativeArrayOptions options = NativeArrayOptions.ClearMemory) where T : struct
        {
            if (size < 0)
            {
                Debug.LogError("EnsureExactSize must be called with a non-negative size.");
                return;
            }
